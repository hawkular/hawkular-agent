...
import org.hawkular.agent.monitor.scheduler.polling.dmr.ReadAttributeOrResourceOperationBuilder;
...
public class AvailJMXTaskGroupRunnable implements Runnable {
    ...
    public void run() {
        ...
        try (JBossASClient client = new JBossASClient(mccFactory.createClient())) {
            ...
            for (ModelNode operation : this.operations) {
                ...
                final AvailJMXTask task = (AvailJMXTask) group.getTask(operationIndex);
    ...
    private Avail getAvailFromResponse(final ModelNode response, final AvailJMXTask task) {
        Avail avail;
        final ModelNode result = JBossASClient.getResults(response);
